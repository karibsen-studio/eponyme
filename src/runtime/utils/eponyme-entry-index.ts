...
/**
 * Bumped whenever this module changes what it would store for the same configuration - a wider
 * `INDEXABLE_TYPES`, a different `foldEponymeIndexValue`, another row shape.
 */
const INDEX_ALGORITHM = 3

/** Option keys that describe how a field looks, never what it stores. */
...
/** Deliberately not `schemaFingerprint`, which covers names and types only. */
...
/**
 * Only top-level fields are covered, which is the same reach as `orderBy`: a field nested in a section, a
 * tab or an array is not addressable by a single key, so neither sorting nor filtering can name one.
 */
export function buildEponymeIndexRows(
  ...
  // A Map rather than an array: a tag list normalized to one spelling can still fold two suggestions onto
  // the same key, and the table's primary key would reject the duplicate.
  ...
  for (const version of ['draft', 'published'] as const) {
    ...
    // Both versions are recorded: a reference held only by a draft still breaks if its target is taken
    // away.
