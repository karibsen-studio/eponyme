...
export default defineEventHandler(async (event) => {
  ...
  if (!route?.submissions) throw createError({ statusCode: 404, statusMessage: 'Eponyme submission not found.' })
  ...
  if (!route.submissionId) {
    ...
    if (deleted === undefined) throw createError({ statusCode: 404, statusMessage: 'Eponyme form not found.' })
    return { deleted }
  }

  if (!await service.deleteSubmission(route.name, route.submissionId))
    throw createError({ statusCode: 404, statusMessage: 'Eponyme submission not found.' })
  return { deleted: true }
})
