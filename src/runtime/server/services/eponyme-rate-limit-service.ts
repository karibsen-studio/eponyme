...
function resolveCounter(mount: string): EponymeRateLimitCounter | undefined {
  ...
  try {
    // Resolved from the unprefixed root storage: `getMount` is one of the few methods `prefixStorage` does
    // not rewrite, so asking a namespaced handle would answer with whatever is mounted at the root instead
    // of with the mount being looked for.
    ...
    // `base` is the driver's own prefix, applied inside its methods and so absent from a direct command.
    ...
    // A mount that cannot be resolved is not a reason to fail a request: the database path is still there,
    // and it is the one that would have been used without Redis at all.
    return undefined
  }
}
