...
describe('field.seo', () => {
  ...
  it('labels every field with a message key, so a translated dashboard translates the section too', () => {
    const fields = field.seo().options.fields
    // Des cles, pas du texte : ce fichier est lu par Node quand la configuration est chargee,
    // ou la locale n'existe pas encore. `humanizeLabel` les traduit au rendu.
    const labels = ['seo.title', 'seo.description', 'seo.image', 'seo.shareSameAsPage', 'seo.ogTitle', 'seo.ogDescription'] as const
    expect(Object.values(fields).map(field => field.options.label)).toEqual(labels)
    expect(field.seo().options.label).toBe('seo.label')
    for (const key of [...labels, 'seo.label' as const]) expect(eponymeEnglishMessages[key]).toBeDefined()
  ...
  it('renders those keys through the catalogue, and leaves an application label alone', () => {
    expect(humanizeLabel('title', 'seo.ogTitle')).toBe(eponymeEnglishMessages['seo.ogTitle'])
    expect(humanizeLabel('title', 'Referencement')).toBe('Referencement')
    expect(humanizeLabel('ogTitle')).toBe('Og Title')
  })
